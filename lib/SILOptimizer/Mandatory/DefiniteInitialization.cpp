...
void LifetimeChecker::injectActorHops() {
  ...
  auto injectExecutorHopAfter = [&](SILInstruction *insertPt) -> void {
    ...
    SILBuilderWithScope::insertAfter(insertPt, [&](SILBuilder &b) {
      ...
      // Borrow the actor.  If the value is an address (as it is
      // in delegating inits), we must also load.
      SILValue actor;
      if (val->getType().isAddress())
        actor = b.createLoadBorrow(genLoc, val);
      else
        actor = b.createBeginBorrow(genLoc, val);
      ...
      if (TheMemory.isNonDelegatingInit()) {
